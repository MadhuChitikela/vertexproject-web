...
const hexToRgb = (hex: string) => {
    const defaultColor = { r: 102, g: 102, b: 102 }; // fallback #666666
    if (!/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/.test(hex)) return defaultColor;
    const normalizeHex = hex.length === 4 ? '#' + hex[1] + hex[1] + hex[2] + hex[2] + hex[3] + hex[3] : hex;
    return {
        r: parseInt(normalizeHex.slice(1, 3), 16),
        g: parseInt(normalizeHex.slice(3, 5), 16),
        b: parseInt(normalizeHex.slice(5, 7), 16)
    };
};
