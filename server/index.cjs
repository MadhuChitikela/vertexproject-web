...
app.post('/api/inquiry', async (req, res) => {
    ...
    const adminMailOptions = {
        ...
        replyTo: email, // Click reply to talk to customer immediately
    ...
    try {
        ...
        res.status(200).json({ success: true, message: 'Emails sent successfully to both parties' });
    } catch (error) {
        console.error('❌ SMTP Error:', error);
        res.status(500).json({ success: false, error: 'Failed to dispatch emails' });
    }
});
